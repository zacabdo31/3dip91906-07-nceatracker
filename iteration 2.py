...
# --- Utility function to clear current screen ---
    ...
    def main_menu(self):
        ...
        tk.Button(self.root, text="Add Assessment", command=self.add_assessment_window, width=20).grid(row=1, column=0, padx=5, pady=5)
        tk.Button(self.root, text="Remove Assessment", command=self.remove_assessment_window, width=20).grid(row=2, column=0, padx=5, pady=5)
        tk.Button(self.root, text="View Assessments", command=self.view_assessments_window, width=20).grid(row=3, column=0, padx=5, pady=5)
        tk.Button(self.root, text="View Total Credits", command=self.show_credits_window, width=20).grid(row=4, column=0, padx=5, pady=5)
        tk.Button(self.root, text="Save Data", command=self.save_data, width=20).grid(row=5, column=0, padx=5, pady=5)
    ...
    def add_assessment_window(self):
        ...
        # Title
        tk.Label(self.root, text="Add New Assessment", font=("Arial", 14, "bold")).grid(row=0, column=0, columnspan=2, pady=10)
        ...
        tk.Label(self.root, text="Credits:").grid(row=2, column=0, sticky="w")
        credits_entry = tk.Entry(self.root)
        credits_entry.grid(row=2, column=1)

        # Date input
        tk.Label(self.root, text="Due Date (YYYY-MM-DD):").grid(row=3, column=0, sticky="w")
        due_date_entry = tk.Entry(self.root)
        due_date_entry.grid(row=3, column=1)

        # Status dropdown
        tk.Label(self.root, text="Status (completed/not_completed):").grid(row=4, column=0, sticky="w")
        ...
        def save_assessment():
            ...
            try:
                ...
                # Validation checks
                if not name or not due_date or not status or not assessment_type:
        ...
        tk.Button(self.root, text="Save", command=save_assessment).grid(row=6, column=0, pady=10)
        tk.Button(self.root, text="Back", command=self.main_menu).grid(row=6, column=1, pady=10)
    ...
    def remove_assessment_window(self):
        ...
        for i, a in enumerate(self.manager.assessments, 1):
            tk.Label(self.root, text=f"{i}. {a}").grid(row=i, column=0, sticky="w")

        # Input for choosing which assessment to delete
        tk.Label(self.root, text="Enter number to remove:").grid(row=len(self.manager.assessments)+1, column=0, sticky="w")
        ...
        tk.Button(self.root, text="Remove", command=remove_item).grid(row=len(self.manager.assessments)+2, column=0, pady=10)
        tk.Button(self.root, text="Back", command=self.main_menu).grid(row=len(self.manager.assessments)+2, column=1, pady=10)
    ...
    def view_assessments_window(self):
        ...
        # Back button
        tk.Button(self.root, text="Back", command=self.main_menu).grid(row=len(self.manager.assessments)+1, column=0, pady=10)
    ...
    def show_credits_window(self):
        ...
        tk.Label(self.root, text=f"Total Credits: {total}").grid(row=1, column=0, sticky="w")
        tk.Label(self.root, text=f"Completed Credits: {completed}").grid(row=2, column=0, sticky="w")

        tk.Button(self.root, text="Back", command=self.main_menu).grid(row=3, column=0, pady=10)
